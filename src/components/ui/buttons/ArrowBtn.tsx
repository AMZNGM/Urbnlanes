...
export default function ArrowBtn({ className, onClick, direction = 'left' }: { className?: string; onClick?: () => void; direction?: 'left' | 'right' }) {
  return (
    <button onClick={onClick} className={`${className} ${direction === 'right' ? 'rotate-180 rtl:rotate-0' : 'rtl:rotate-180'} `}>
      <RippleEffect className="group relative w-full h-full overflow-hidden justify-center items-center grid bg-current/10 hover:bg-current/20 rounded-xl text-current transition-colors duration-300 px-4 py-2 cursor-pointer">
