...
export default function BehindTheFigures() {
  return (
    <section className="relative w-dvw overflow-hidden bg-bg text-text px-4 py-16">
      <AnimIn className="space-y-4 bg-main/10 rounded-lg text-center p-4">
        <AnimText as="h5" delay={0.3} className="font-light text-[9dvw] max-md:text-5xl rtl:leading-55 tracking-tight mb-4">
          <TText tKey="db.whoweare.behindthefigures[0].title" />
        </AnimText>

        <AnimText as="p" delay={0.6} className="font-light text-2xl md:text-3xl rtl:leading-10">
          <TText tKey="db.whoweare.behindthefigures[0].tagline" />
        </AnimText>

        <MotionLine delay={0.75} className="opacity-75" />

        <AnimText delay={0.9} className="max-w-4xl font-light text-base md:text-lg normal-case leading-relaxed mx-auto pt-2">
        ...
        <div className="gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-8">
          {db.whoweare.behindthefigures.map((figure, index) => (
            <AnimIn key={index} delay={0.2 * index} className="group relative">
              <AnimIn className="relative w-full h-full bg-main/10 rounded-lg p-4 max-md:p-2">
                ...
                <AnimText delay={0.8 + 0.2 * index} className="text-2xl rtl:leading-10 mb-4">
