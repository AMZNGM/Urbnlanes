...
export default function VideoModal({ videos, closeModel }: { videos: string | null; closeModel: () => void }) {
  return (
    <AnimatePresence>
      {videos && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeModel} className="z-50 fixed inset-0 bg-black/95">
            ...
            <div onClick={(e) => e.stopPropagation()} className="w-full max-w-6xl aspect-video">
              {isYouTubeUrl(videos) ? (
                <iframe
                  ...
                  allowFullScreen
                  className="w-full h-full bg-main/25 rounded-2xl"
                />
              ) : (
                <video src={videos} controls autoPlay className="w-full h-full bg-main/25 rounded-2xl" />
              )}
            </div>
          </motion.div>
        </motion.div>
