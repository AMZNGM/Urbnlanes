...
export default memo(function NewsCard({ article, look = 'default', priority = false }: NewsCardProps) {
  ...
  if (look === 'mono') {
    return (
      <RippleEffect as="article" className="group w-full h-full overflow-hidden bg-text rounded-lg text-bg">
        <Link href={`/media-center-news/${article.id}`} className="w-full h-full flex flex-col">
          <ImageIn src={imageSrc} alt={article.title || 'News article'} priority={priority} className="rounded-lg" divClassName="rounded-lg overflow-hidden" />

          <div className="top-4 left-4 absolute bg-main/25 backdrop-blur-xl rounded-lg font-mono text-[10px] text-text tracking-widest px-3 py-2">
      ...
      <RippleEffect as="article" className="group w-full h-full overflow-hidden bg-text rounded-lg text-bg">
        <Link href={`/media-center-news/${article.id}`} className="w-full h-full flex flex-col">
          ...
          <div className="flex flex-col gap-2 py-2">
            ...
            <div className="w-fit bg-main/50 rounded-lg font-mono text-[10px] text-bg tracking-widest px-2 py-1">
              <TText tKey={`news.${article.type}`} />
            </div>
  ...
  return (
    <RippleEffect as="article" dir="ltr" className="group rounded-lg max-md:py-2">
      <Link href={`/media-center-news/${article.id}`} className="block relative w-full h-full">
        <ImageIn
          ...
          priority={priority}
          className="scale-100!"
          divClassName="h-100! blur-none! overflow-hidden rounded-lg"
        />

        <div className="right-0 bottom-0 md:bottom-83 group-hover:bottom-0 left-0 absolute flex flex-col justify-between bg-bg/50 backdrop-blur-xl rounded-lg text-text md:scale-80 group-hover:scale-100 transition-all duration-500 m-4 p-4">
