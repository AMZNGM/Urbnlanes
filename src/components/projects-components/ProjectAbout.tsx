...
export default function ProjectAbout({ project }: { project: Project }) {
  ...
  return (
    <section className="relative w-dvw overflow-hidden bg-text text-bg px-4 max-md:px-2 py-24 max-md:py-12">
      <div className={`max-w-6xl mx-auto ${project.shortDesc || project.description || project.brochure ? 'grid lg:grid-cols-3 gap-12' : 'space-y-12'}`}>
        <div className={`space-y-8 col-span-1 max-lg:col-span-3 ${project.shortDesc || project.description || project.brochure ? '' : `hidden`}`}>
          ...
          <div className="space-y-6 font-light text-bg text-lg normal-case leading-relaxed">
            ...
            {project.brochure && (
              <AnimIn delay={0.6} className="w-fit flex items-center gap-2 border rounded-lg mt-8 p-2">
                <Download size={16} />
        ...
        <AnimIn className="relative max-lg:h-100 min-h-120 overflow-hidden col-span-2 max-lg:col-span-3 bg-main/25 rounded-lg">
        ...
        <AnimIn className="relative grid grid-cols-4 col-span-3 bg-main/25 rounded-lg text-center">
          ...
          <motion.div
            className="absolute inset-0 bg-main/25 rounded-lg mix-blend-difference"
            initial={{ width: '0%' }}
