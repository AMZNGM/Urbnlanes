...
export default function CareersList({ className, dark }: { className?: string; dark?: boolean }) {
  ...
  return (
    <section className={`relative w-dvw min-h-[80dvh] overflow-hidden px-4 max-md:px-2 py-24 ${dark ? 'bg-bg text-text' : 'bg-text text-bg'} ${className}`}>
      {careersData.departments.map((dept: any, index: number) => {
        ...
        return (
          <AnimIn delay={index * 0.1} key={dept.id} className="border-b">
            <button onClick={() => toggle(dept.id)} className="group w-full flex justify-between items-center-safe gap-6 py-12 cursor-pointer">
              ...
              <h3 className="max-md:text-2xl text-4xl">
            ...
            <AnimatePresence>
              {isExpanded && (
                ...
                >
                  <div className="max-w-5xl gap-4 grid grid-cols-2 mx-auto pt-4 pb-12">
